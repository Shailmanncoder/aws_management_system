 ...
 * responses are checked against this list before being used to build SDK endpoints or console
 * links, so no value can steer a request at a non-AWS host. Update when AWS launches regions.
 *
 * Lives in `lib` (not `server`) because client-reachable code — console deep links in the guided
 * walkthroughs — needs the same check. It is pure data with no server imports.
 */
