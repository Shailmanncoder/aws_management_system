...
export default async function WeeklyPage({ searchParams }: PageProps<"/weekly">) {
  ...
  return <div className="mx-auto max-w-5xl space-y-6"><PageHeader title="Weekly summary" description={`All workspace accounts · ${summary.start.toISOString().slice(0,10)} to ${summary.end.toISOString().slice(0,10)} (end exclusive, UTC)${offset === 0 ? " · week in progress" : ""}`} actions={<PrintSummary />} />
